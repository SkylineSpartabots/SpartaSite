...
const HistoryPage = ({year, robotName, robotNameBlurb, splashImage, robotImage, robotInfo, gameImage, gameName, gameInfo, performance}) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
      ...
      <div className="bg-slate-100 flex flex-col gap-8 items-center p-8">
        ...
        <div className="sm:flex sm:flex-col flex-1 px-6 py-8 max-w-[1000px] bg-white rounded gap-4">
          ...
          <div className="hidden sm:flex flex-row flex-initial px-6 py-8 max-w-[1000px] bg-white rounded gap-4">
            {(performance.map(info => {return (<InfoBlock
              title={info.title}
              text={info.text}
            />)}).reduce((a, b) => [a,(<div className="border-l-[1px] border-gray-300 min-h-[350px] mx-4"></div>),b]))}
          </div>
          <div className="sm:hidden flex flex-col flex-initial px-6 py-8 max-w-[1000px] bg-white rounded gap-4">
            {(performance.map(info => {return (<InfoBlock
              title={info.title}
              text={info.text}
